...
async function handleSubmit() {
  ...
  try {
    ...
    // Erro inesperado (não deve acontecer, pois sendToBlockchain trata erros)
...
async function registrarComFeedback(dados: any, processoId?: string) {
  console.log('=ä Iniciando envio para blockchain...');
...
=æ [sendToBlockchain] Chamando formatPayload...
...
=æ [sendToBlockchain] Payload formatado com sucesso
< [sendToBlockchain] Preparando requisição HTTP POST para /api/v1/blockchain/register
ó [sendToBlockchain] Enviando requisição...
...
=ä [sendToBlockchain] Objeto de retorno montado: {...}
